...
class Column extends React.Component {
  ...
  clear(item) {
    const data = this.state.data.filter(dataItem => dataItem !== item);
    this.setState({ data });
  }
...
export default class Masonry extends React.Component {
  ...
  clearAll() {
    this.state.columns.forEach(col => col.clearAll());
  }

  clear(item) {
    this.state.columns.forEach(col => col.clear(item));
  }
  ...
  scrollTo = params => {
    this.scrollView.current?.scrollTo(params);
  };
